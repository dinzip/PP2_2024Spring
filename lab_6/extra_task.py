...
def print_console(path): 
    ...
    for i in content:
        if current != active:
            print(Back.LIGHTBLUE_EX + i.name, end='')
        else:
            print(Back.LIGHTMAGENTA_EX + i.name, end='')
        print(Back.LIGHTBLUE_EX + ' '* (80-len(i.name)-1), end = '.')
        print(Back.BLACK + '')
        current += 1
...
def show_file(file):
    name, content = file
    print()
    print(Fore.YELLOW+ f'Content of file "{name}":', end='')
    print(Back.BLACK + '')
    print(Fore.GREEN + content)
    if len(content)==0:
        print(Fore.LIGHTRED_EX + 'empty')
    return []
...
while True:
    ...
    if len(name_and_content_of_file) != 0:
        name_and_content_of_file = show_file(name_and_content_of_file)
